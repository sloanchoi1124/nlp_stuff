...
class BerkeleyAligner():
	...
	de_vocab = set()
        ...
        for aligned_sentence in aligned_sents:
            en_vocab.update(aligned_sentence.words)
            de_vocab.update(aligned_sentence.mots)
	...
	en_vocab.add(None)
        ...
        for i in range(num_iters):
            ...
            for aligned_sentence in aligned_sents:
                de_sentence = [None] + aligned_sentence.mots
                en_sentence = [None] + aligned_sentence.words
                l = len(aligned_sentence.mots)
                m = len(aligned_sentence.words)
        ...
        en_vocab = set()
        for aligned_sentence in aligned_sents:
            en_vocab.update(aligned_sentence.mots)
            de_vocab.update(aligned_sentence.words)

        de_vocab.add(None)
	en_vocab.add(None)
        translation_table2 = defaultdict(lambda: defaultdict(lambda: 1.0e-12))

        alignment_table2 = defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: defaultdict(lambda: 1.0e-12))))
        ...
        for i in range(num_iters):
            ...
            for aligned_sentence in aligned_sents:
                de_sentence = [None] + aligned_sentence.words
                en_sentence = [None] + aligned_sentence.mots
                l = len(aligned_sentence.words)
                m = len(aligned_sentence.mots)
                total_count = defaultdict(float)
