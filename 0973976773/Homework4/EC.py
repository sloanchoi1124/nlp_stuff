...
import nltk
import A
from nltk.align import Alignment, AlignedSent, IBMModel1
...
class BetterBerkeleyAligner():
    ...
    def align(self, align_sent):
        ...
        for j, en_word in enumerate(align_sent.words):
            max_align_prob = (self.t[en_word][None] * self.q[0][j + 1][m][l], None)
            for i, de_word in enumerate(align_sent.mots):
                # Find out the maximum probability
                max_align_prob = max(max_align_prob, (
                self.t[en_word][de_word] * self.q[i + 1][j + 1][m][l], i))

            
            if max_align_prob[1] is not None:
                best_alignment.append((j, max_align_prob[1]))

        new_sent = AlignedSent(align_sent.words, align_sent.mots, best_alignment)
	return new_sent
